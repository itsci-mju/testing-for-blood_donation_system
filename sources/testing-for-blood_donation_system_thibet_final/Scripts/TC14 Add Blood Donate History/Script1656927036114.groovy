...
String xlPath_tc = "D:/Project/Result/TC14 Add Blood Donate History/TC14 Add Blood Donate HistoryResult.xlsx";
String xlPath_ts = "D:/Project/Result/TC14 Add Blood Donate History/TS14 Add Blood Donate HistoryResult.xlsx";
String xlPath_td = "D:/Project/Result/TC14 Add Blood Donate History/TD14 Add Blood Donate HistoryResult.xlsx";
String xlPath ="D:/Project/KDF/TC14 Add Blood Donate History.xlsx";
	...
	if(xTCdata[i][4].equalsIgnoreCase('y')){
		...
		for(int j = 1; j < xTDdata.length; j++){
			...
			if(xTDdata[j][1].equalsIgnoreCase('y')){
				...
				CustomKeywords.'webTest.NewKeywords.edit_inputs'(xTSdata[8][5], xTDdata[j][4])
				CustomKeywords.'webTest.NewKeywords.button_click'(xTSdata[9][5])
				CustomKeywords.'webTest.NewKeywords.edit_inputs'(xTSdata[10][5], xTDdata[j][5])
				CustomKeywords.'webTest.NewKeywords.edit_inputs'(xTSdata[11][5], xTDdata[j][6])
				CustomKeywords.'webTest.NewKeywords.button_click'(xTSdata[12][5])
					try {
					...
					xTDdata[j][8] = actual_text
					if(actual_text.equals(xTDdata[j][7])){
						xTDdata[j][9] = "Pass"
					}else{
						   xTDdata[j][9] = "Fail"
						   xTDdata[j][10] = "- Not Match Expected Result -"
						   xTDdata[j][11] = "TC14_TD"+ j
						   CustomKeywords.'webTest.NewKeywords.Takescreen'("D:/Project/Result/Capture/TC14_TD"+ j +".png")
					}
				}
				 WebUI.closeBrowser()	
			}	
